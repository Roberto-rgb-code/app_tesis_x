...
# Carga de datos
data_file = st.file_uploader("Sube tu archivo (CSV, Excel o JSON)", type=["csv", "xlsx", "xls", "json"])
if not data_file:
...
try:
    if file_extension == ".csv":
        ...
        st.error("Formato de archivo no soportado. Por favor, sube un archivo CSV, Excel o JSON.")
        st.stop()
except Exception as e:
    st.error(f"Error al leer el archivo: {str(e)}. Asegúrate de que tenga la estructura correcta.")
    st.stop()
